...
export type RegisterOptions = {
    /** HuggingFace 토큰. HTTP에서는 x-hf-token 헤더, stdio에서는 HF_TOKEN 환경변수에서 주입된다. */
    hfToken?: string
...
/**
 * 도구/프롬프트/리소스를 서버 인스턴스에 등록한다.
 * stdio 진입점과 HTTP 라우트가 공유하며, 트랜스포트별 차이는 options로만 전달된다.
 */
export function registerAll(server: McpServer, options: RegisterOptions) {
    server.registerTool(
        'greet',
        {
        ...
        'geocode',
        {
            ...
            inputSchema: z.object({
                city: z
                    .string()
                    .min(2)
                    .describe('도시명 또는 우편번호 (예: 삿포로, Sapporo, 04524)'),
                count: z
                    .number()
                    .int()
                    .min(1)
    ...
    server.registerTool(
        ...
        async ({ prompt, num_inference_steps }) => {
            const token = options.hfToken

            if (!token) {
                return errorResult(
                    options.transport === 'http'
                        ? 'HuggingFace 토큰이 없습니다. https://hf.co/settings/tokens 에서 토큰을 발급받아 MCP 클라이언트 설정의 headers에 "x-hf-token"으로 추가하거나, 서버의 HF_TOKEN 환경변수를 설정하세요.'
    ...
    server.registerResource(
        ...
        SERVER_INFO_URI,
        {
            title: 'Server Info',
            description:
                '이 MCP 서버의 기본 정보(이름, 버전, 제공 도구)를 반환합니다.',
            mimeType: 'text/plain'
        },
        async (uri) => ({
            contents: [
                {
                    ...
                    text: [
                        ...
                        'auth:',
                        options.transport === 'http'
                            ? '- x-hf-token 헤더로 HuggingFace 토큰 전달 (없으면 서버의 HF_TOKEN 환경변수 사용)'
                            : '- HF_TOKEN 환경변수로 HuggingFace 토큰 전달'
                    ].join('\n')
                }
            ]
        })
